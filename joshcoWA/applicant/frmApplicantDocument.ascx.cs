...
{
    public partial class frmApplicantDocument : System.Web.UI.UserControl
    {
        ...
        {
            foreach (GridViewRow row in this.gvDocument.Rows)
            {
                ((Button)row.FindControl("btnAppUpload")).Visible = true;
        ...
        public void HtmlAnchorDeleteDoc_Click(Object sender, EventArgs e)
        {
            ...
            HiddenField hfApplicantDocumentID = (HiddenField)row.FindControl("hfApplicantDocumentID");

            this.sdsDocument.DeleteParameters["ApplicantDocumentID"].DefaultValue = hfApplicantDocumentID.Value;
        ...
        protected void gvDoc_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (this.hfApplicantSuccessful.Value != "0")
                {
        ...
        public void UploadFiles()
        {
            ...
            foreach (GridViewRow row in this.gvDocument.Rows)
            {
                ...
                {
                    byte[] bytes = Convert.FromBase64String(((HiddenField)row.FindControl("hfUploadfileData")).Value);

                    SaveFile(((HiddenField)row.FindControl("hfUploadDocumentCategoryID")).Value,
                        ((HiddenField)row.FindControl("hfUploadname")).Value,
                        ((HiddenField)row.FindControl("hfUploadcontentType")).Value,
                        Convert.ToInt32(((HiddenField)row.FindControl("hfUploadsize")).Value),
                       bytes);
                } 
            }
        }
        ...
        protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Remove")
            {
                ...
                ((HiddenField)row.FindControl("hfUploadDocumentCategoryID")).Value = null;
                ((HiddenField)row.FindControl("hfUploadname")).Value = null;
                ((HiddenField)row.FindControl("hfUploadcontentType")).Value = null;
                ((HiddenField)row.FindControl("hfUploadsize")).Value = null;
                ((HiddenField)row.FindControl("hfUploadfileData")).Value = null;

                ((System.Web.UI.WebControls.FileUpload)row.FindControl("fuAppUpload")).Visible = true;
                ((LinkButton)row.FindControl("lnkDocumentName")).Visible = false;
                ((LinkButton)row.FindControl("lnkDeleteDoc")).Visible = false;
                ((Button)row.FindControl("btnAppUpload")).Visible = false;
                ((LinkButton)row.FindControl("lnkDocumentName")).Text = "";
                ((Label)row.FindControl("lblRequired")).Text = "*";

                //this.sdsDocument.DeleteParameters["ApplicantDocumentID"].DefaultValue = e.CommandArgument.ToString();
                //this.sdsDocument.Delete();
                    ...
                    if (((HiddenField)row.FindControl("hfUploadname")).Value == "")
                    {
                        ...
                        if (file.ContentLength > 0)
                        {
                            ...
                            if (Session["ApplicantID"] == null)
                            {
                                ...
                                ((HiddenField)row.FindControl("hfUploadcontentType")).Value = contentType;
                                ((HiddenField)row.FindControl("hfUploadsize")).Value = size.ToString();
                                ((HiddenField)row.FindControl("hfUploadfileData")).Value = Convert.ToBase64String(fileData);
